...
// Pulls finished recipe objects out of a partially-streamed {"recipes":[...]} payload.
// ponytail: rescans the full text on every delta; fine at ~4KB payloads
function extractRecipes(text) {
  const complete = [];
  let partialName = "";
  const rIdx = text.indexOf('"recipes"');
  const start = rIdx === -1 ? -1 : text.indexOf("[", rIdx);
  ...
  for (let i = start + 1; i < text.length; i++) {
    ...
    if (c === '"') inStr = true;
      ...
      if (depth === 0 && objStart !== -1) {
        try { complete.push(JSON.parse(text.slice(objStart, i + 1))); } catch { /* skip malformed object */ }
        objStart = -1;
      }
    }
  ...
  if (objStart !== -1) {
    const m = text.slice(objStart).match(/"name"\s*:\s*"([^"]*)/);
    if (m) partialName = m[1];
  }
  return { complete, partialName };
}

// Reads an Anthropic SSE stream, calling onText(fullTextSoFar) as text deltas arrive.
// Returns the complete text. Throws on a mid-stream error event.
...
// Streams a recipe request to the Anthropic API; onUpdate({complete, partialName})
// fires as recipes finish. Resolves with the completed recipes, throws on any failure.
async function streamRecipes(apiKey, prompt, onUpdate, fetchFn = fetch) {
  ...
  const text = await readSSEText(res.body.getReader(), t => onUpdate(extractRecipes(t)));
  const { complete } = extractRecipes(text);
  if (!complete.length) throw new Error("No recipes in the response. Try again.");
  return complete;
}
...
Rules: difficulty is Easy/Medium/Hard; macros are realistic per-serving integers; 4-7 steps each; 3 recipes meaningfully different in cuisine or method.`;
    setStep("results");
    try {
      await streamRecipes(apiKey, prompt, ({ complete, partialName }) => {
        setRecipes(complete);
  ...
  if (step === "results") return (
    <div style={{ padding: "16px" }}>
      ...
      {loading && recipes.length < 3 && (
        <div style={{ ...card, marginBottom: 14, padding: "18px", animation: "fadeUp 0.4s ease both" }}>
          <div style={{ fontSize: 11, fontWeight: 700, color: T.mintDark, letterSpacing: 1, marginBottom: 8, animation: "pulse 1.4s ease infinite" }}>
            OPTION {recipes.length + 1} · CLAUDE IS WRITING…
          </div>
          <div style={{ fontSize: 18, fontWeight: 800, color: T.black, minHeight: 24 }}>
            {streamName}<span style={{ color: T.mintDark, animation: "pulse 1s ease infinite" }}>▍</span>
          </div>
        </div>
      )}
    </div>
  ...
  return (
    <>
      ...
      <div style={{ maxWidth: 430, margin: "0 auto", height: "100vh", background: T.bg, position: "relative", overflow: "hidden", fontFamily: "-apple-system, 'SF Pro Display', 'Segoe UI', system-ui, sans-serif" }}>
        ...
        {/* Scrollable content */}
        <div ref={scrollRef} style={{ height: `calc(100vh - 36px - 72px - ${apiKey ? 36 : 40}px)`, overflowY: "auto", overflowX: "hidden" }}>
          {tab === "home"    && <HomeScreen />}
          {tab === "plan"    && <PlanScreen />}
